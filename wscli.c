...
void wsclient_handshake(wsclient *client);
...
char* msg_login = "{\"id\":1,\"method\":\"call\",\"params\":[1,\"login\",[\"\",\"\"]]}";
char* msg_database = "{\"id\":2,\"method\":\"call\",\"params\":[1,\"database\",[]]}";
char* msg_network_broadcast = "{\"id\":3,\"method\":\"call\",\"params\":[1,\"network_broadcast\",[]]}";
char* msg_history = "{\"id\":4,\"method\":\"call\",\"params\":[1,\"history\",[]]}";
char* msg_get_chain_id = "{\"id\":5,\"method\":\"call\",\"params\":[2, \"get_chain_id\", []]}";
char* msg_set_callback = "{\"id\":6,\"method\":\"call\",\"params\":[2,\"set_block_applied_callback\",[0]]}"; //5
char* msg_get_global_properties = "{\"id\":7,\"method\":\"call\",\"params\":[2, \"get_global_properties\", []]}"; //6
char* msg_get_dynamic_global_properties = "{\"id\":8,\"method\":\"call\",\"params\":[2, \"get_dynamic_global_properties\", []]}"; //7
char* msg_lookup_account_names = "{\"id\":9,\"method\":\"call\",\"params\":[2, \"lookup_account_names\", [[\"nathan\"]]]}"; //8


int loop_send(wsclient *c, int n) {
   ...
   case 1:
      ...
      break;
   case 2:
      data = msg_network_broadcast;
      break;
   case 3:
      data = msg_history;
      break;
   case 4:
      data = msg_get_chain_id;
      break;
   case 5:
      data = msg_set_callback;
      break;
   case 6:
      data = msg_get_global_properties;
      break;
   case 7:
      data = msg_get_dynamic_global_properties;
      break;
   default:
      data = msg_lookup_account_names;
      break;
   }

   fprintf(stdout, "request: %s\n", data);
   wsclient_send(c, data);
}
...
int main(int argc, char **argv) {
   ...
   wsclient *client = wsclient_new("ws://10.7.0.216:11011"); // echo.websocket.org");
   if (!client) {
...
inline void wsclient_in_data(wsclient *client, char in) {
   ...
   if (wsclient_complete_frame(client, current) == 1) 
   {
      if (current->fin == 1) {
         ...
         if ((current->opcode & 0x08) == 0x08) {
            ...
            wsclient_dispatch_message(client, current);
...
void wsclient_dispatch_message(wsclient *c, wsclient_frame *current) 
{
